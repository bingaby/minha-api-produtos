...
app.post('/api/produtos', upload.array('imagens', 10), async (req, res) => {
    ...
    try {
        const uploadedImages = imagens && imagens.length > 0 ? await Promise.all(imagens.map(async (file) => {
...
app.put('/api/produtos/:id', upload.array('imagens', 10), async (req, res) => {
    ...
    try {
        ...
        const query = `
            ...
            RETURNING *
        `;
        const values = [nome, descricao, preco, link, categoria, loja, uploadedImages, id];
        const result = await client.query(query, values);
        client.release();
